...
Node* Node::insert_before(int d) {
  ...
  if(previous != NULL)	// make sure there is a previous node
	previous->next = n;
...
Node* Node::insert_after(int d) {
  ...
  if(next != NULL) // make sure there is a next node
	  next->previous = n;
...
void Node::delete_before(){
  ...
  if(previous->previous != NULL) // make sure the new previous node exists
...
{
  if(next == NULL){ // make sure there is a previous node
	  cout <<"No Next Node\n";
	  return;
  }
  ...
  if(next->next != NULL) // make sure the new next node exists
